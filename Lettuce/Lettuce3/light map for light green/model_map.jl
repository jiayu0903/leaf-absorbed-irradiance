...
const tON = ("09:20:00 PM",
             "09:24:00 PM",
             "09:28:00 PM")

const t0s = DateTime("06/09/2023 09:16:50 PM", dateformat"d/m/y H:M:S p")
const t0c = DateTime("06/09/2023 07:40:04 PM", dateformat"d/m/y H:M:S p")
const tst = DateTime("06/09/2023 09:20:00 PM", dateformat"d/m/y H:M:S p")
...
function Load_temps()
    # Build a mask
    img = load("Mask map-light green.bmp")
    s = size(img)
    img = img[2:(s[1]-1), 2:(s[2]-1)]
    tmp = Gray.(img)
    mask = tmp .> 0.0
    s = size(mask)
