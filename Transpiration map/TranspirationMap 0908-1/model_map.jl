...
const dt = 5809.0 # Time difference between the camera and sensor 
...
function Load_temps()
 ...
 maskr = reshape(mask, s[1] * s[2], 1)
    ...
    ts .-= ts[1] + 41.55 # Time recorded before the sensor started 
...
function Run()
    ...
    for i in 1:N
        ...
        # Fit the model
        fit = curve_fit(model, x, y, [y[1], y[1] + 5, 20.0])

        # if maximum(y) - minimum(y) > 1.0
        #     scatter(x, y)
