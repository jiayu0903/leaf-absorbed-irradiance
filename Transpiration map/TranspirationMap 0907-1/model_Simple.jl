...
function plotSim(p, env, tps)
    ...
    p1 = plot(tps.time, tps.Tl, label="Tl obs.")
    plot!(tps.time, view(tmp, 1, :), label="Tl mod.")
...
function deltaT(p, env, tps)
    ...
    p1 = plot(tps.time, tps.Tl, label="Observed", xlabel="Time (s)", 
              ylab=L"T_{leaf}\,\,(°C)", xlim=[0.0, 31.0],
              seriestype=:scatter, markersize=10)
    plot!(tps.time, view(tmp, 1, :), label="Modelled", color=:orange, linewidth=4)
    ...
    pp = similar(p)
    pp[:] = p[:]
    Q = collect(0.0:10.0:250.0)
    Tl = similar(Q)

    cs = range(HSL(colorant"green"), stop=HSL(colorant"red"), length=length(Q))
    ...
    for i = eachindex(Q)
        ...
        scatter!([30.0], [tmp[1, end]], color=cs[i], label=false)
        plot!(t, view(tmp, 1, :), color=cs[i], label=false)
        Tl[i] = tmp[end]
    end
    p1 |> display
    p2 = scatter(Q, Tl, color=cs, legend=false, xlabel="Absorbed irradiance (W/m²)", ylab=L"Modelled\,\,T_{leaf} (°C)")
    a = Q \ (Tl .- Tl[1])
    plot!(Q, a .* Q .+ Tl[1]) |> display

    pg = plot(p1, p2, ncol=2, sizes=(1280, 640), bottom_margin = 10mm, left_margin = 10mm,
              tickfontsize=16, guidefontsize=16)

    png(pg, "LightTemps.png")
